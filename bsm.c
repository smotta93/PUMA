 ...
 */
  ...
  if (cp_sign > 0)
  {
    for (i=0; i < n_flavors; i++)
      for (j=0; j < n_flavors; j++)
        _H[i][j] = _H0_template[i][j] * inv_E  +  Ve*epsilon_m[i][j] + pow(E,2) * a_v[i][j] + pow(E,5) * c_v[i][j];		  
	    /* _H[i][j] = pow(E, 2 ) * a_v[i][j] - pow(E,5) * c_v[i][j]; */
  }
  else
  {
    for (i=0; i < n_flavors; i++)
      for (j=0; j < n_flavors; j++)
        _H[i][j] = conj(_H0_template[i][j] * inv_E   +  Ve*epsilon_m[i][j] -  pow(E,2) * a_v[i][j] + pow(E,5) * c_v[i][j]);		  
	    /* _H[i][j] = conj(- pow(E,2) * a_v[i][j] - pow(E,5) * c_v[i][j]); */
...
/* quando for rodar só o puma adicione a matriz de massa nessa parte para não precisar definir um novo parâmetro
	_H[0][1] = _H[0][1] + (m_sqr * inv_E)/(2);
    _H[0][2] = _H[0][2] + (m_sqr * inv_E)/(2);
etc*/
